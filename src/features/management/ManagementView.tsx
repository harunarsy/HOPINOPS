          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            ...
            <div className="section-card">
              ...
              <form className="roster-form" onSubmit={handleCreateRoster} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '12px', marginTop: '16px' }}>
                ...
                <label style={labelStyle}>Petugas<select required disabled={Boolean(rosterUsersError)} value={rosterProfileId} onChange={(event) => setRosterProfileId(event.target.value)} style={{ ...inputStyle, marginTop: '4px' }}><option value="">Pilih petugas aktif</option>{usersList.filter((entry) => entry.role !== 'INVESTOR' && !entry.force_pin_change).map((entry) => <option key={entry.id} value={entry.id}>{entry.display_name} · {taskLabel(entry.role)}</option>)}</select></label>
            ...
            <div className="section-card">
              ...
              {!loading && !viewError && overtime.length === 0 ? (
                ...
                <table className="management-table overtime-table" style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse', fontSize: '13px' }}>
                  <thead><tr style={{ borderBottom: '1px solid #cddcd4', color: '#476058' }}><th style={{ padding: '8px' }}>Petugas</th><th style={{ padding: '8px' }}>Tanggal</th><th style={{ padding: '8px' }}>Durasi terdeteksi</th><th style={{ padding: '8px' }}>Kredit</th><th style={{ padding: '8px' }}>Catatan</th><th style={{ padding: '8px' }}>Status</th><th style={{ padding: '8px' }}>Aksi</th></tr></thead>
                  <tbody>{overtime.map((claim) => {
                    const attendance = claim.attendance_records;
                    return <tr key={claim.id} style={{ borderBottom: '1px solid #eef3f0' }}><td style={{ padding: '8px', fontWeight: 600 }}>{attendance?.profiles?.display_name ?? 'Pengguna'}</td><td style={{ padding: '8px' }}>{attendance?.work_date}</td><td style={{ padding: '8px' }}>{claim.raw_extra_minutes} menit</td><td style={{ padding: '8px' }}>{claim.credited_hours} jam</td><td style={{ padding: '8px', color: claim.status === 'CANDIDATE' ? '#547066' : 'inherit', maxWidth: '260px' }}>{claim.reason || '—'}</td><td style={{ padding: '8px' }}><span className={`tag ${claim.status === 'APPROVED' ? 'good' : claim.status === 'REJECTED' ? 'neutral' : 'warn'}`}>{taskLabel(claim.status)}</span></td><td style={{ padding: '8px' }}>{claim.status === 'CANDIDATE' && (attendance?.profile_id === user.id ? <span className="muted">Reviewer lain diperlukan</span> : <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}><button type="button" className="outline-button" onClick={() => { setReviewNote(''); setOvertimeReview({ claim, decision: 'APPROVED' }); }} style={{ padding: '5px 8px', fontSize: '11px' }}>Setujui</button><button type="button" className="outline-button" onClick={() => { setReviewNote(''); setOvertimeReview({ claim, decision: 'REJECTED' }); }} style={{ padding: '5px 8px', fontSize: '11px', color: '#b91c1c', borderColor: '#fecaca' }}>Tolak</button></div>)}</td></tr>;
                  })}</tbody>
                </table>
              </div>}
            </div>
        ...
        {rosterEdit && (
          <Dialog titleId="edit-roster-title" title="Ubah Jadwal" onClose={() => { if (actionLoading !== 'roster-edit') setRosterEdit(null); }}>
            <form onSubmit={handleUpdateRoster} style={{ display: 'grid', gap: '12px' }}>
              ...
              <label style={labelStyle}>Petugas<select required value={rosterEdit.profile_id} onChange={(event) => setRosterEdit({ ...rosterEdit, profile_id: event.target.value })} style={{ ...inputStyle, marginTop: '4px' }}><option value="">Pilih petugas aktif</option>{usersList.filter((entry) => entry.role !== 'INVESTOR' && !entry.force_pin_change).map((entry) => <option key={entry.id} value={entry.id}>{entry.display_name} · {taskLabel(entry.role)}</option>)}</select></label>
