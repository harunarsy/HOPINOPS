...
## Katalog Produksi: Daftar Barang Asli BAR + KITCHEN — 17 September 2026

- Pengguna menyetujui pemasukan daftar barang asli (BAR + KITCHEN) ke production `naanarmoktmsumkxmjvj` melalui jalur resmi katalog (`rpc_create_item_auto` / `rpc_archive_item_auto`, actor OWNER `HARUN`), bukan SQL mentah; guard script menolak target selain production ref dan mewajibkan `--confirm-production`.
- HASIL AKHIR: BAR 53 item aktif (`BAR-009`–`BAR-061`) dan KITCHEN 75 item aktif (`KIT-007`–`KIT-081`), urutannya disesuaikan dengan daftar final dari pengguna (BAR: `Kopi` → `BAR-009` … `Bulpen` → `BAR-061`; KITCHEN: `Beras` → `KIT-007` … `Jeruk Nipis` → `KIT-081`) dan terverifikasi 100% cocok (nama, satuan, batas minimum). Item dummy lama diarsipkan: `BAR-001`–`BAR-008`, `KIT-001`–`KIT-006`. Sequence berikutnya: BAR-062 / KIT-082.
- Konvensi konversi yang disetujui pengguna: `kg` → gram (×1000), `L` → liter, item tanpa satuan → pcs, "Kopi Arabika" → 1000 gram; item tanpa batas minimum diisi taksiran dan dapat diubah supervisor/operator lewat aplikasi.
- Cleanup cycle terbengkalai (disetujui pengguna): 5 cycle kosong (3–9 Sep; nol assignment aktif) ditandai `RESET` setelah backup; trigger 0022 mempromosikan snapshot pending otomatis. Tidak ada lagi cycle terbuka maupun pending catalog.
- BUG DITEMUKAN & DIPERBAIKI: `next_item_display_code` memakai `lpad(text, 3, '0')` yang MEMOTONG nilai ≥ 1000 (1020 → '102'), sehingga kode bentrok (`KIT-121`) dan RPC create gagal berulang. Perbaikan = `supabase/migrations/0033_item_code_sequence_padding.sql` (pad minimal 3 digit, tidak pernah memotong), diuji di staging (`KIT-999`/`KIT-1000` benar) dan diterapkan ke production oleh pengguna via Supabase SQL Editor; assertion regresi ditambahkan ke `supabase/tests/database.test.sql` (plan 32 → 34).
- Koreksi penomoran satu kali (disetujui pengguna; snippet `supabase/snippets/2026-09-17_fix_kitchen_item_codes.sql` + `2026-09-17_renumber_item_codes.sql`): kode katalog asli dirapikan berurutan sesuai urutan pembuatan; item dummy arsip tidak diubah. Backup: `backups/seed-items-2026-09-17T14-35-40-070Z/`, `backups/seed-items-2026-09-17T15-06-43-445Z/`, `backups/cycle-cleanup-2026-09-17T14-38-12-991Z/`.
- FIX NOMOR HANGUS (disetujui pengguna): `pending_catalog_promote` menulis seluruh snapshot dengan `INSERT ... ON CONFLICT`, sehingga trigger `trg_items_display_code` mengalokasikan kode untuk SETIAP baris — termasuk item lama yang berakhir sebagai update (tambah 1 item = sequence naik sejumlah isi area). Migration `0034_promote_allocates_new_items_only.sql` mengubahnya menjadi update-langsung untuk item lama dan insert hanya untuk item baru. Terbukti di staging: buat item = sequence +1 (sebelumnya +7), arsip item tidak menaikkan sequence sama sekali; semantik promote lain tidak berubah.
- Urutan kode final diterapkan ke production lewat satu paste (migration 0034 + snippet `2026-09-17_reorder_item_codes.sql`, peta nama→kode divalidasi 1:1 terhadap isi database sebelum DML, trigger kode permanen di-bypass terkontrol seperti koreksi sebelumnya). Snippet bersifat idempoten dan transaksional.
- Catatan: penandaan cycle `RESET` tidak memiliki RPC resmi sehingga dieksekusi via service-role terkontrol dan didokumentasikan di entri ini; assignment dan histori absensi/stok tidak diubah.
- Jalur migration staging (`pnpm run db:staging:migrate`) gagal dari workstation saat ini (direct IPv6 + Management API 403 `login role`); 0033 diterapkan ke staging via pooler psql + baris ledger `public.hopin_schema_migrations`. Perlu `supabase login` ulang untuk memulihkan jalur resmi.
