...
psql_cmd() {
  psql -X -v ON_ERROR_STOP=1 -h "$host" -p "$port" -d "$database" "$@"
}
...
if [ -z "$shift_code" ]; then
  printf '%s\n' 'No free (shift, area) cycle slot left today for the concurrency fixture' >&2
  exit 1
fi
...
set check_in_event_id = '$checkin_event_id'
...
begin
  select count(*) into checkout_events
  from public.attendance_events
  where attendance_id = '$attendance_id'
    and event_type = 'CHECK_OUT';
  select count(*) into audit_rows
  from public.audit_events
  where action = 'SELF_EMERGENCY_CHECKOUT'
    and entity_id = '$attendance_id';
  select count(*) into receipt_rows
  ...
  if checkout_events <> 1 or audit_rows <> 1 or receipt_rows <> 1 then
    raise exception 'Concurrent self-emergency invariant failed: checkout %, audit %, receipt %', checkout_events, audit_rows, receipt_rows;
  end if;
  raise notice 'PASS: concurrent self-emergency created one checkout, audit and receipt';
end \$\$;
SQL
...
# Cleanup committed fixture so reruns and sibling suites see a clean disposable DB.
# Order is children-first (events cascade from records); audit history stays.
psql_cmd -v ON_ERROR_STOP=1 <<SQL
begin;
-- Disposable-DB hygiene only: triggers (incl. append-only) are bypassed to
-- remove this run's own committed fixture. Never point this script elsewhere.
