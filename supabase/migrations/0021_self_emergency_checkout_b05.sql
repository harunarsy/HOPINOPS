-- 0021_self_emergency_checkout_b05.sql
-- B05: Operator boleh check-out darurat untuk dirinya sendiri (self-service).
-- Manager-on-behalf (rpc_emergency_checkout) tidak diubah. Target diambil dari sesi
-- server (attendance milik sendiri yang masih terbuka); tidak menerima target arbitrary.
-- Tidak otomatis menyetujui kehadiran, menyelesaikan stok, atau memfinalisasi payroll.
...
begin
  if p_expected_attendance_version is null or p_expected_attendance_version <= 0
  ...
  -- Attendance milik sendiri yang masih terbuka hari ini (self-service, tanpa target arbitrary).
  ...
  v_request := jsonb_build_object(
    'attendance_id', v_attendance.id, 'expected_attendance_version', p_expected_attendance_version,
    'reason', v_reason
  );
  insert into public.workflow_idempotency (
    actor_user_id, outlet_id, action, idempotency_key, request_json
  ) values (
    p_actor_id, p_outlet_id, 'SELF_EMERGENCY_CHECKOUT', p_idempotency_key, v_request
  ...
  if v_attendance.status = 'CHECKED_OUT' or v_attendance.check_out_event_id is not null then
    raise exception using errcode = '55000', message = 'ALREADY_CHECKED_OUT: Check-out sudah tercatat. Bila assignment belum selesai, gunakan pemulihan penyelesaian assignment.';
  end if;
  if v_attendance.status not in ('CHECKED_IN', 'REVIEW_REQUIRED')
     or v_attendance.check_in_event_id is null then
  ...
  insert into public.attendance_events (
    ...
    null, null, 100, jsonb_build_array('SELF_EMERGENCY_CHECKOUT'),
  ...
  perform public.log_audit_event(
    p_actor_id, 'SELF_EMERGENCY_CHECKOUT', 'attendance_records', v_attendance.id::text,
    p_outlet_id, p_actor_id, v_before,
    v_response || jsonb_build_object(
